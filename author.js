...
div.dataset.id = f.id;
div.innerHTML =   <input type="text" class="form-control form-control-sm frontInput" value="${escapeHTML(f.front||'')}" placeholder="Front">   <input type="text" class="form-control form-control-sm backInput" value="${escapeHTML(f.back||'')}" placeholder="Back">   <input type="text" class="form-control form-control-sm explanationInput" value="${escapeHTML(f.explanation||'')}" placeholder="Explanation">   <button class="btn btn-sm btn-danger delFlashBtn"><i class="bi bi-trash"></i></button>  ;
...
div.dataset.id = q.id;
div.innerHTML =    <input type="text" class="form-control form-control-sm questionInput mb-1" value="${escapeHTML(q.question||'')}" placeholder="Question">   <div class="d-flex gap-1 mb-1">   ${q.choices.map((c,j)=><input type="text" class="form-control form-control-sm choiceInput" data-index="${j}" value="${escapeHTML(c||'')}" placeholder="Choice ${j+1}">).join("")}   </div>   <select class="form-select form-select-sm correctInput mb-1">   ${q.choices.map((_,j)=><option value="${j}" ${q.answer===j?"selected":""}>${j+1}</option>).join("")}   </select>   <textarea class="form-control form-control-sm explanationInput" placeholder="Explanation">${escapeHTML(q.explanation||"")}</textarea>   <button class="btn btn-sm btn-danger delQuizBtn mt-1"><i class="bi bi-trash"></i> Delete</button>   ;
