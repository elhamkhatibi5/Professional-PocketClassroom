...
function loadAuthorForm() {
  if (!currentCapsule) {
    // اگر capsule نساخته شد، یک پیش‌فرض ایجاد کن
  ...
  (currentCapsule.quiz || []).forEach((q, i) => {
    ...
    div.innerHTML = `
      ...
      <div class="d-flex gap-1 mb-1">
        ${q.choices.map((c, j) => `<input type="text" class="form-control form-control-sm choiceInput" value="${escapeHTML(c)}" placeholder="Choice ${j+1}">`).join("")}
      </div>
...
saveCapsuleBtn.addEventListener("click", () => {
  ...
  // آپدیت Library و Learn (اگر توابع موجود باشند)
  if (typeof renderLibrary === "function") renderLibrary(searchCapsule.value);
